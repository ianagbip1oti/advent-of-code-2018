...
Point = Struct.new(:x, :y, :vx, :vy) do
  ...
  def self.parse(s)
    Point.new *s.scan(/-?\d+/).map(&:to_i)
  end
end

INPUT = DATA.map { |line| Point.parse line }.to_a

def yspread(points)
  mn, mx = points.map(&:y).minmax

  mx - mn
end

def pretty_print(points)
  mn_x, mx_x = points.map(&:x).minmax
  mn_y, mx_y = points.map(&:y).minmax

  (mn_y..mx_y).each do |y|
    (mn_x..mx_x).each do |x|
      print points.find { |p| p.x == x && p.y == y }.nil? ? ?. : ?#
    end
    puts
  end
end
...
def parts
  ...
  spread = yspread(state)
  prev_state = state.dup

  time_taken = 0.step do |t|
    state = state.map(&:tick)

    new_spread = yspread state

    break t if new_spread > spread
