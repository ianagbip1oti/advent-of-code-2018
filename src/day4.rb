...
INPUT = DATA.each_line.map(&:chomp).to_a
...
def minutes
  ...
  INPUT.sort.each do |line|
    id_match = /Guard #(\d+) begins shift/.match line
    asleep_match = /:(\d+)\] falls asleep/.match line
    wakes_match = /:(\d+)\] wakes up/.match line

    if id_match
      guard_id = id_match[1].to_i
    elsif asleep_match
      asleep = asleep_match[1].to_i
    else
      wakes = wakes_match[1].to_i
      minutes[asleep...wakes].each { |m| m << guard_id }
    end
  end
...
def part1
  ...
  minutes_count = MINUTES.reduce({}) do |acc, m|
    acc.merge(m.group_by(&:itself)) { |k, o, v| o + v }
  end.transform_values(&:size)

  sleepiest_guard = minutes_count.max_by { |k, v| v }[0]

  count_minutes = MINUTES.map { |m| m.count(sleepiest_guard) }.each_with_index.max[1]

  p "#{sleepiest_guard}x#{count_minutes}=#{sleepiest_guard * count_minutes}"
end

def part2
  ms = MINUTES.map do |m|
    m.group_by(&:itself).transform_values(&:size).max_by { |k, v| v }
  end.select{|m| !m.nil? }.each_with_index.max_by { |k, v| k[1] }

  guard = ms[0][0]
  min = ms[1]

  p "#{guard}x#{min}=#{guard * min}"
end
